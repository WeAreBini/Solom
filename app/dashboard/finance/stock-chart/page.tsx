...
export default function StockChartDemoPage() {
  ...
  // Build indicator query params based on settings
  const getIndicatorParams = useCallback(() => {
    const params: string[] = [];
    ...
    if (indicatorSettings.bollingerBands.enabled) {
      params.push('bollingerBands');
    }
    
    return params.join(',');
  }, [indicatorSettings]);

  // Build indicator options for API query
  const indicatorOptions = {
    indicators: getIndicatorParams() ? getIndicatorParams().split(',').filter(Boolean) : [],
    smaPeriod: indicatorSettings.sma.enabled ? indicatorSettings.sma.period : undefined,
    emaPeriod: indicatorSettings.ema.enabled ? indicatorSettings.ema.period : undefined,
    rsiPeriod: indicatorSettings.rsi.enabled ? indicatorSettings.rsi.period : undefined,
    macdFast: indicatorSettings.macd.enabled ? indicatorSettings.macd.fastPeriod : undefined,
    macdSlow: indicatorSettings.macd.enabled ? indicatorSettings.macd.slowPeriod : undefined,
    macdSignal: indicatorSettings.macd.enabled ? indicatorSettings.macd.signalPeriod : undefined,
    bbPeriod: indicatorSettings.bollingerBands.enabled ? indicatorSettings.bollingerBands.period : undefined,
    bbStdDev: indicatorSettings.bollingerBands.enabled ? indicatorSettings.bollingerBands.stdDev : undefined,
  };
  ...
  const indicatorData: IndicatorData | undefined = data?.indicators ? {
    ...
    bollingerBands: data.indicators.bollingerBands,
      ...
      <main className="container mx-auto px-4 py-8">
        <div className="mb-8">
          ...
          <p className="mt-2 text-muted-foreground">
            Interactive stock charts with SMA, EMA, RSI, MACD, and Bollinger Bands
          </p>
        </div>
        ...
        <div className="grid gap-6 lg:grid-cols-4">
          ...
          <div className="lg:col-span-3 space-y-6">
            <StockChart
              ...
              showBollingerBands={indicatorSettings.bollingerBands.enabled}
              smaPeriod={indicatorSettings.sma.period}
              emaPeriod={indicatorSettings.ema.period}
              rsiPeriod={indicatorSettings.rsi.period}
              bollingerPeriod={indicatorSettings.bollingerBands.period}
              bollingerStdDev={indicatorSettings.bollingerBands.stdDev}
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                <div className="grid gap-4 sm:grid-cols-2">
                  ...
                  <div className="space-y-2">
                    <h4 className="font-medium text-cyan-600">Bollinger Bands</h4>
                    <p className="text-sm text-muted-foreground">
                      Volatility bands showing upper and lower boundaries. Prices near upper band may indicate overbought, near lower band oversold.
                    </p>
                  </div>
                  <div className="space-y-2">

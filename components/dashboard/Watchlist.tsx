...
export function Watchlist({ symbols, onRemove }: WatchlistProps) {
  ...
  const prevStockDataRef = useRef<string>("");
  ...
  // Initialize stocks from fetched data - only when new symbols are added
  useEffect(() => {
    if (!data) return;
    
    const watchlistStocks = data.filter(s => symbols.includes(s.symbol));
    const stocksKey = watchlistStocks.map(s => s.symbol).join(",");
    
    // Only update if the set of stocks has changed
    if (stocksKey !== prevStockDataRef.current) {
      prevStockDataRef.current = stocksKey;

...
export function StockQuoteDetail({ symbol, onClose }: StockQuoteDetailProps) {
  ...
  // Use stock price as initial value, then allow local state for real-time updates
  const [price, setPrice] = useState<number | null>(null);
  const [previousPrice, setPreviousPrice] = useState<number | null>(null);

  // Sync price when stock data loads - intentional pattern for real-time simulation
  useEffect(() => {
    if (stock?.price !== undefined && price === null) {
